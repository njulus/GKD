...
class MyNetwork(nn.Module):
    def __init__(self, args, flag_general=False, flag_bias=True):
        ...
        self.fc = nn.Linear(in_features = self.cfg[12], out_features = number_of_classes, bias=flag_bias)

        if flag_general:
            self.fc_teacher = nn.Linear(in_features = self.cfg[12], out_features = number_of_classes, bias=flag_bias)
    ...
    def forward(self, x, flag_embedding=False, flag_both=False, flag_general=False):
        y = self.conv1(x)
        y = self.bn1(y)
        y = F.relu(y, inplace = True)
        y = self.layers(y)
    ...
    def get_teacher_classifier_params(self):
        if not self.flag_general:
            return None
        else:
            modules = [self.fc_teacher]
            for i in range(len(modules)):
                for j in modules[i].parameters():
                    yield j